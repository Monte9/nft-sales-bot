  ...
  async saveCollectionData(
    ...
    const UPSERT_COLLECTION = gql`
      mutation UpsertCollection(
        ...
        $updateCollectionRow: [collection_update_column!]!
      ) {
        insert_collection(
          objects: $collection
          on_conflict: {
            update_columns: $updateCollectionRow
    ...
    try {
      const data: Mutation_Root = await this.graphQLClient.request(
        UPSERT_COLLECTION,
        variables
      )
      const collectionData: Collection_Mutation_Response =
